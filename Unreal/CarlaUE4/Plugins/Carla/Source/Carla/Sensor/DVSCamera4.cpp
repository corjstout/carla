...
ADVSCamera4::ADVSCamera4(const FObjectInitializer &ObjectInitializer)
...
FActorDefinition ADVSCamera4::GetSensorDefinition()
{
...
void ADVSCamera4::Set(const FActorDescription &Description)
{
...
void ADVSCamera4::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera4::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera4::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera4 Stream Send");
    /** Send the events **/
...
void ADVSCamera4::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera4::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera4::DVSEventArray ADVSCamera4::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera4::DVSEventArray events;
