...
ADVSCamera10::ADVSCamera10(const FObjectInitializer &ObjectInitializer)
...
FActorDefinition ADVSCamera10::GetSensorDefinition()
{
...
void ADVSCamera10::Set(const FActorDescription &Description)
{
...
void ADVSCamera10::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera10::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera10::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera10 Stream Send");
    /** Send the events **/
...
void ADVSCamera10::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera10::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera10::DVSEventArray ADVSCamera10::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera10::DVSEventArray events;
