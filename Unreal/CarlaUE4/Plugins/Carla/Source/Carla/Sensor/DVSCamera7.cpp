...
ADVSCamera7::ADVSCamera7(const FObjectInitializer &ObjectInitializer)
...
FActorDefinition ADVSCamera7::GetSensorDefinition()
{
...
void ADVSCamera7::Set(const FActorDescription &Description)
{
...
void ADVSCamera7::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera7::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera7::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera7 Stream Send");
    /** Send the events **/
...
void ADVSCamera7::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera7::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera7::DVSEventArray ADVSCamera7::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera7::DVSEventArray events;
