...
ADVSCamera3::ADVSCamera3(const FObjectInitializer &ObjectInitializer)
...
FActorDefinition ADVSCamera3::GetSensorDefinition()
{
...
void ADVSCamera3::Set(const FActorDescription &Description)
{
...
void ADVSCamera3::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera3::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera3::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera3 Stream Send");
    /** Send the events **/
...
void ADVSCamera3::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera3::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera3::DVSEventArray ADVSCamera3::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera3::DVSEventArray events;
