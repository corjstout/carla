...
ADVSCamera9::ADVSCamera9(const FObjectInitializer &ObjectInitializer)
...
FActorDefinition ADVSCamera9::GetSensorDefinition()
{
...
void ADVSCamera9::Set(const FActorDescription &Description)
{
...
void ADVSCamera9::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera9::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera9::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera9 Stream Send");
    /** Send the events **/
...
void ADVSCamera9::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera9::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera9::DVSEventArray ADVSCamera9::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera9::DVSEventArray events;
