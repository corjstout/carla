...
ADVSCamera5::ADVSCamera5(const FObjectInitializer &ObjectInitializer)
...
FActorDefinition ADVSCamera5::GetSensorDefinition()
{
...
void ADVSCamera5::Set(const FActorDescription &Description)
{
...
void ADVSCamera5::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera5::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera5::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera5 Stream Send");
    /** Send the events **/
...
void ADVSCamera5::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera5::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera5::DVSEventArray ADVSCamera5::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera5::DVSEventArray events;
