...
ADVSCamera8::ADVSCamera8(const FObjectInitializer &ObjectInitializer)
...
FActorDefinition ADVSCamera8::GetSensorDefinition()
{
...
void ADVSCamera8::Set(const FActorDescription &Description)
{
...
void ADVSCamera8::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera8::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera8::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera8 Stream Send");
    /** Send the events **/
...
void ADVSCamera8::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera8::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera8::DVSEventArray ADVSCamera8::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera8::DVSEventArray events;
