...
FActorDefinition ADVSCamera2::GetSensorDefinition()
{
...
void ADVSCamera2::Set(const FActorDescription &Description)
{
...
void ADVSCamera2::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera2::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera2::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera2 Stream Send");
    /** Send the events **/
...
void ADVSCamera2::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera2::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera2::DVSEventArray ADVSCamera2::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera2::DVSEventArray events;
