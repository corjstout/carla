...
ADVSCamera6::ADVSCamera6(const FObjectInitializer &ObjectInitializer)
...
FActorDefinition ADVSCamera6::GetSensorDefinition()
{
...
void ADVSCamera6::Set(const FActorDescription &Description)
{
...
void ADVSCamera6::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  TRACE_CPUPROFILER_EVENT_SCOPE(ADVSCamera6::PostPhysTick);
  ...
  /** DVS Simulator **/
  ADVSCamera6::DVSEventArray events = this->Simulation(DeltaTime);

  if (events.size() > 0)
  {
    TRACE_CPUPROFILER_EVENT_SCOPE_STR("ADVSCamera6 Stream Send");
    /** Send the events **/
...
void ADVSCamera6::ImageToGray(const TArray<FColor> &image)
{
  /** Sanity check **/
...
void ADVSCamera6::ImageToLogGray(const TArray<FColor> &image)
{
  /** Sanity check **/
  if (image.Num() != (this->GetImageHeight() * this->GetImageWidth()))
    return;
...
ADVSCamera6::DVSEventArray ADVSCamera6::Simulation (float DeltaTime)
{
  /** Array of events **/
  ADVSCamera6::DVSEventArray events;
